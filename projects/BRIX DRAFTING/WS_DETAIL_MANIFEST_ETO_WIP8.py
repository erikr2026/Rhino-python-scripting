"""
WS Detail Manifest Manager (WIP8 - streamlined from WIP7).
...
Rhino 8's ScriptEditor command (type ScriptEditor, open this file, press F5)
-- NOT RunPythonScript, which invokes the legacy IronPython 2 engine and
will fail on this codebase's syntax/imports.

Behavior is unchanged from WIP7 (see the WIP7 README for the full feature
description). This revision is a cleanup pass only: extracted repeated
Eto.Forms/selection/command-fallback boilerplate into helper functions,
trimmed unused exception variables, and added/clarified a few docstrings.
No UI, ordering, or logic changes were made. See the reporting message
delivered alongside this file for the itemized diff against WIP7.
"""
...
def _resolve_reference_path(path, ws_dir, doc_dir):
    """
    Resolves a (possibly relative/incomplete) reference path extracted from a
    worksession file to an existing file on disk. Tries, in order: the path
    as-is, relative to the worksession file's directory (full path, then
    basename-only), then relative to the active document's directory (full
    path, then basename-only). Returns the resolved absolute path, or None
    ...
    candidates = []
    if ws_dir:
        candidates.append(os.path.abspath(os.path.join(ws_dir, path)))
        candidates.append(os.path.abspath(os.path.join(ws_dir, os.path.basename(path))))
    if doc_dir:
        candidates.append(os.path.abspath(os.path.join(doc_dir, path)))
        candidates.append(os.path.abspath(os.path.join(doc_dir, os.path.basename(path))))
...
def _get_all_scene_objects():
    """
    Returns the enumerator of active/reference/normal/locked document objects
    used by both the manifest-apply and show-all-in-detail routines (kept as
    a single settings definition so the two stay in sync).
    """
    ...
    return sc.doc.Objects.GetObjectList(settings)

def _select_object_ids(obj_ids):
    """Clears the current selection and selects exactly the given object ids (skipping any that no longer resolve)."""
...
def _run_command_on_ids(obj_ids, command_name):
    """
    Selects the given object ids and runs a command-line macro (e.g.
    "_HideInDetail" / "_ShowInDetail") on them, then clears the selection.
    Used as the fallback for write-protected worksession reference objects
    whose attributes can't be committed directly. No-op if obj_ids is empty.
    """
    if not obj_ids:
        return
    _select_object_ids(obj_ids)
    rs.Command(command_name, echo=False)
    sc.doc.Objects.UnselectAll()
...
def _apply_manifest_core(detail_obj):
    ...
    all_objs = _get_all_scene_objects()
    ...
    _run_command_on_ids(failed_to_hide, "_HideInDetail")
    _run_command_on_ids(failed_to_show, "_ShowInDetail")
...
def _make_button(text):
    """
    Creates an Eto Button with the given label text. Controls are instantiated
    with no constructor keyword arguments and Text assigned afterward -- this
    matches WIP7's existing pattern noted as required for PythonNet runtime
    compatibility; left unchanged here, only de-duplicated into one helper.
    """
    btn = forms.Button()
    btn.Text = text
    return btn
...
class DetailManifestDialog(forms.Form):
    ...
    def __init__(self):
        ...
        self.btn_refresh = _make_button("Refresh Active Detail")
        self.btn_load_ws = _make_button("Load References from .WS / .RWS")
        self.btn_save = _make_button("Set / Overwrite Manifest")
        self.btn_apply = _make_button("SINGLE DETAIL APPLY MANIFEST (HIDE OTHERS)")
        self.btn_apply_all = _make_button("ALL DETAILS APPLY MANIFEST")
        self.btn_add = _make_button("Add Selected to Manifest")
        self.btn_remove = _make_button("Remove Selected from Manifest")
        self.btn_clear = _make_button("Clear / Reset Manifest")
    ...
    def on_load_ws_click(self, sender, e):
        ...
        for path in extracted:
            found_path = _resolve_reference_path(path, ws_dir, doc_dir)
            if found_path:
    ...
    def on_add_click(self, sender, e):
        ...
        if selected_ids:
            _select_object_ids(selected_ids)
