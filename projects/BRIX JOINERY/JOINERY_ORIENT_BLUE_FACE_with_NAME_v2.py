    ...
    for set_name in sorted_set_names:
        ...
        for obj_id, seq_suffix, obj_name in items_in_set:
            ...
            if not blue_faces:
                ...
                fallback_name = obj_name if obj_name.strip() != "" else str(obj_id)
            ...
            if final_transform.IsValid:
                ...
                if new_obj_id != System.Guid.Empty:
                    ...
                    print("Copied and nested '{}' at X: {:.3f}, Y: {:.3f}, Z: 0.470 (Set: {})".format(
                        obj_name if obj_name.strip() != "" else str(obj_id), 
                        current_x,
                        current_y,
                        set_name
                    ))
                    ...
                    try:
                        amp = rg.AreaMassProperties.Compute(geometry)
                        if amp:
                            orig_centroid = amp.Centroid
                    except Exception:
                        pass
                    
                    if not orig_centroid:
                        orig_centroid = geometry.GetBoundingBox(True).Center
